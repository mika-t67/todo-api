...
- `GET /api/tasks/jobs/logs` で実行履歴を取得可能。フロントエンド側では、ログ表示中は5秒ごとに自動でポーリング(再取得)し、ジョブの実行状況をリアルタイムに近い形でモニタリングできるようにしています。
...
| XSS | JSON APIのためHTMLを直接返さない設計。フロントエンド(React)はJSX内で値を自動エスケープするため、通常の描画ではスクリプト注入のリスクを大幅に低減。ただし `dangerouslySetInnerHTML` や外部HTMLの直接挿入は行っておらず、これらを使用する場合は別途サニタイズが必要になる点に留意。`helmet`によるヘッダー設定は補完的な防御層 |
...
- MongoDB AtlasのNetwork Accessは、開発環境での接続確認を容易にするため `0.0.0.0/0 `を設定しています。本番環境では接続元IPを限定する運用が必要です。


## 設計上の工夫

- **スケジュールジョブの障害時にもサーバーを継続稼働させる設計**
  ジョブ本体のエラーだけでなく、失敗ログの記録処理自体が失敗するケースも考慮し、二重の例外処理を実装。
- **XSS・CSRF対策におけるリスクとトレードオフを考慮した設計**
  Reactの自動エスケープやHelmet等を利用しつつ、JWT認証方式との組み合わせを考慮してCSRF対策の必要性・適用範囲を判断。
- **ジョブ実行状況のポーリングによる準リアルタイム可視化**
  5秒間隔でジョブログを取得し、ジョブの実行結果を画面に自動反映。Socket.ioを使用せず、実装の複雑さと通信頻度のバランスを考慮。
- **用途に応じたPostgreSQLとMongoDBの使い分け**
  ユーザー・タスク・ジョブログなど整合性が重要な構造化データにはPostgreSQL、チャット履歴には柔軟なドキュメント形式で保存できるMongoDBを採用。
- **Prisma 7の仕様変更への対応**
  Prisma 7で必要となったドライバーアダプターの構成に対応し、`@prisma/adapter-pg`を利用してPostgreSQLとの接続を実装。